...
wxDialog(parent, wxID_ANY, wxT("主窗口"), wxDefaultPosition, wxSize(MINIMUM_WIDTH, MINIMUM_HEIGHT), 
           (wxDEFAULT_DIALOG_STYLE & ~wxCLOSE_BOX) | wxRESIZE_BORDER) // Cannot be closed
{
...
void DialogMain::Setup()
{
    ...
    m_tree = new wxTreeCtrl(this, ID_LAYOUT_TREE);

    // All "groups"
    wxStaticBox* const treeGroup = new wxStaticBox(this, wxID_ANY, wxT("浏览窗口:"));
    wxStaticBox* const propertyGroup = new wxStaticBox(this, wxID_ANY, wxT("当前选择窗口的可用属性:"));

    // Button(s)
    m_addWindow = new wxButton(this, ID_ADD_WINDOW, wxT("添加窗口"));
    //m_LoadLayout = new wxButton(this, ID_LOAD_LAYOUT, wxT("Add layout"));
    m_saveSelection = new wxButton(this, ID_SAVE_SELECTION, wxT("保存当前选择"));
    ...
    wxStaticBox* const contentGroup = new wxStaticBox(this, wxID_ANY, wxT("文本:"));
    ...
    wxStaticBox* const typeGroup = new wxStaticBox(this, wxID_ANY, wxT("类型:"));
    ...
    // common modes
    const wxString options[] = {wxT("显示所有"), wxT("只显示公用")};
    m_displayModeRadio = new wxRadioBox(this, ID_SKINS_DISPLAY, wxT("皮肤与属性的多重选择显示模式:"), wxDefaultPosition, wxDefaultSize, 2, options);
    ...
    wxStaticBox* const skinGroup = new wxStaticBox(this, wxID_ANY, wxT("皮肤:"));	
...
void DialogMain::CreateGrid()
{
    ...
    m_grid->SetColLabelValue(COLUMN_NAME, wxT("属性名称"));
...
wxString DialogMain::GetFakePropertyCommonValue(const wxString& propertyName) const
{
    ...
    if(currentWindow)
    {
        ...
        if(m_document->GetSelection())
        {
            ...
            for (Selection::Boxes::iterator boxIt = m_document->GetSelection()->GetBoxes().begin(); boxIt != m_document->GetSelection()->GetBoxes().end(); ++boxIt)
            {
...
void DialogMain::MapWidgetSelectionPossibleSkins()
{
    ...
    else
    {
        ...
        Selection::Boxes::iterator boxIt = m_document->GetSelection()->GetBoxes().begin();
        ...
        for (; boxIt != m_document->GetSelection()->GetBoxes().end(); ++boxIt)
        {
...
void DialogMain::UpdateSelectionInfo()
{
    ...
    if (m_document && m_document->GetSelection()->Size() == 1)
    {
        ...
        else
		{
			...
			m_editText->SetValue(wxT("当前选择的窗口不支持文本属性"));
		}
        (void)m_displayModeRadio->Enable(false);
...
void DialogMain::OnAddWindow(wxCommandEvent& WXUNUSED(event)) 
{
    try
    {
        ...
        if (windowContext.AcceptsWindowAsChild()) 
        {    
...
void DialogMain::OnSaveSelection(wxCommandEvent& WXUNUSED(event))
{
    ...
    wxFileDialog dialog(this, wxT("打开窗口定义文件"), currentDirectory, wxT(""), wxT("窗口定义文件 (*.xml;*.layout;*.*)|*.xml;*.layout;*.*"), wxSAVE);
...
void DialogMain::OnGridCellChange(wxGridEvent& event)
{
    ...
    // Obtain grid row the user just modified
    const int propertyRow = event.GetRow();

    // Is it a real property or a fake one?
    // NOTE: Fake properties have indices in the range [0, FAKE_PROPERTY_COUNT[
    if (propertyRow >= 0 && propertyRow < FAKE_PROPERTY_COUNT)
    {
    ...
    else
    {
        ...
        const wxString propertyName = m_grid->GetCellValue(propertyRow, COLUMN_NAME);
        const wxString propertyValue = m_grid->GetCellValue(propertyRow, COLUMN_VALUE);
        (void)m_document->SetProperty(StringHelper::ToCEGUIString(propertyName), StringHelper::ToCEGUIString(propertyValue));

        // Set the correct font (normal if property is at default value in all elements in
        // the current selection, bold otherwise)
        SetPropertyValueFont(propertyRow, propertyName, true);
    }
}
...
//------------------------------------------------------------------------
void DialogMain::LayoutAdded (CEGUI::Window* aRoot) 
{
    m_currentRoot = aRoot;
    m_currentWindow = aRoot;
    SyncTree();
...
void DialogMain::SetWidgetsLook(Selection* currentSelection, const CEGUI::String& newLook)
{
    ...
    for (Selection::Boxes::iterator boxIt = currentSelection->GetBoxes().begin(); boxIt != m_document->GetSelection()->GetBoxes().end(); ++boxIt)
    {
