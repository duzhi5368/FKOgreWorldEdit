...
void CELayoutEditor::InitImageHandlers() const
{
    ...
    void CELayoutEditor::DisplaySplashScreen()
    {
        ...
        // NOTE: The image is assumed to be in the current directory of the application
        // NOTE2: An error box will be presented to the user if the splash image isn't found
        wxBitmap bitmap;
    #ifdef __linux__
        const bool loadSplash = bitmap.LoadFile(wxT(CLEDITOR_DATAPATH"/splash.png"), wxBITMAP_TYPE_PNG);
    #else
        const bool loadSplash = bitmap.LoadFile(wxGetCwd() + wxT("\\splash.png"), wxBITMAP_TYPE_PNG);
    #endif
...
void CELayoutEditor::CreateAboutBox()
{
    ...
    info.SetCopyright(wxT("(C) 2005-2007 CEGUI Tools Developer Team"));
...
void CELayoutEditor::OnFatalException()
{
    // Check if wxWidgets was built with debug report support
        ...
        // NOTE: Unlike unhandled exceptions which can be customized, fatal exceptions 
        // are ALWAYS shown to user
        CELEShowMessage(wxT("CELayoutEditor::OnFatalException - CELayoutEditor has encountered a fatal exception and needs to terminate. We're sorry for the inconvenience!"),wxT("CELayoutEditor Fatal Exception"), MB_OK | MB_ICONSTOP | MB_SYSTEMMODAL);
