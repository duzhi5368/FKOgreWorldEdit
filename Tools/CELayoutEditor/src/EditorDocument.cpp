...
bool EditorDocument::OnOpenDocument(const wxString& filename)
{
    ...
    try 
    {
        // Use the full path, because layouts can be loaded from everywhere.
        CEGUI::Window* const root = CEGUI::WindowManager::getSingleton().loadWindowLayout(StringHelper::ToCEGUIString(filename));
        SetActiveLayout (root);

        // Tell observers. Disabled because the view is already notified.
        for (size_t i = 0; i < m_observers.size(); ++i) 
        {
            m_observers [i]->LayoutOpened(root);
        }
...
bool EditorDocument::OnNewDocument () 
{
    ...
    {    
        // Make it active
        CEGUI::Window* const newLayout = CEGUI::WindowManager::getSingletonPtr()->createWindow ("DefaultGUISheet", "Root");
        SetActiveLayout(newLayout);

        // Tell observers. Disabled because the view is already notified.
        for (size_t i = 0; i < m_observers.size(); ++i) 
        {
            m_observers [i]->LayoutAdded(newLayout);
        }

        // Select it
        SelectWindow (newLayout);
...
CEGUI::Window* EditorDocument::AddWindow (CEGUI::Window* aParentWindow, const CEGUI::String& aType, CEGUI::String aName) 
{
    if (!aParentWindow)
    {
        LogError (wxT("EditorDocument::AddWindow - parent should not be NULL."));
        return NULL;
    }
    if( aName.empty() )
    ...
    {   
        ...
        // Delegate; this block can still throw, for example when looknfeel information for automatically generated windows (XX__auto_XX) is missing.
        CEGUI::Window* const newWindow = AddNewWindow (aParentWindow, aType, newName);
...
//------------------------------------------------------------------------
CEGUI::Window* EditorDocument::AddNewWindow (CEGUI::Window* aParentWindow, const CEGUI::String& aType, const CEGUI::String& aName) 
{
    // Validations
    wxASSERT_MSG(aParentWindow != NULL, wxT("Supplied parent window is NULL"));

    LogDebug ( wxT("ParentClass: '%s', newClass: '%s'"), StringHelper::ToWXString(aParentWindow->getType()).c_str(), StringHelper::ToWXString(aType).c_str());

    // Create the window
    CEGUI::Window* const newWindow = CEGUI::WindowManager::getSingleton().createWindow (aType, aName);
    ...
    // Some initializing. Menus need some specific handling...
    ...
    {	// Just put some text in the caption
        ...
        {   // We never explicitly add PopupMenus; when the parent is a MenuItem, we have two options:
            ...
            {   // Create popup menu for this item
                const CEGUI::String look = newWindow->getLookNFeel().substr(0, newWindow->getLookNFeel().find_first_of("/"));
        ...
        {   // Because of the filtering in DialogAddWindow, the parent can only be a Menubar.
            aParentWindow->addChildWindow (newWindow);
        }
    }
    else
    {   // Always add as child when nothing special is going on.
        aParentWindow->addChildWindow (newWindow);
    }
    // If we add a MenuBar, we need to position and size it for a bit.
    if (newWindow->testClassName("Menubar"))
    {
        newWindow->setPosition(CEGUI::UVector2(CEGUI::UDim(0.0f, 0.0f), CEGUI::UDim(0.12f, 0.0f)));
        newWindow->setSize(CEGUI::UVector2(CEGUI::UDim(1.0f, 0.0f), CEGUI::UDim(0.1f, 0.0f)));
    }
    ...
    else if (newWindow->testClassName("DefaultWindow") && aParentWindow->testClassName("TabControl"))
    {
        ...
        newWindow->setSize(CEGUI::UVector2(CEGUI::UDim(1.0f, 0.0f), CEGUI::UDim(1.0f, 0.0f)));
    }
    ...
    {   // Set initial (relative) position in the usual top-left to bottom-right apprauch
        ...
        newWindow->setPosition(CEGUI::UVector2(CEGUI::UDim(newStartXY, 0.0f), CEGUI::UDim(newStartXY, 0.0f)));
        newWindow->setSize(CEGUI::UVector2(CEGUI::UDim(1.0f / ADD_WINDOW_HELPER, 0.0f), CEGUI::UDim(1.0f / ADD_WINDOW_HELPER, 0.0f)));
    }
    // Apply the default font (if set)
    const wxString defaultFont = Options::GetInstancePtr()->GetDefaultFont();
    if (defaultFont != wxT(""))
    {
        newWindow->setProperty("Font", StringHelper::ToCEGUIString(defaultFont));
        if (newWindow->isPropertyPresent("TitlebarFont"))
        {
            newWindow->setProperty("TitlebarFont", StringHelper::ToCEGUIString(defaultFont));
        }
    }
    return newWindow;
}
...
void EditorDocument::InitTabControl (CEGUI::Window* aTabControl)
{
    ...
    if (aTabControl->testClassName("TabControl"))
    {
        ...
        for (int i=0; i<TAB_COUNT; i++)
        {
            ...
            newTab = AddNewWindow (aTabControl, "DefaultWindow", controlName + StringHelper::ToCEGUIString(tabIndex));
            //newTab->setSize(UVector2(UDim(1.0f, 0.0), UDim(1.0f, 0.0)));
            newTab->setText(StringHelper::ToCEGUIString(tabCaptions[i]));    // Overwrite default caption.
            //aTabControl->addChildWindow(newTab);
        }
    }
    /*for (int i=0; i<aTabControl->getChildCount(); i++)
    {
    LogDebug (wxT("Child: '%s'(%s)"), StringHelper::ToWXString(aTabControl->getChildAtIdx(i)->getName()), StringHelper::ToWXString(aTabControl->getChildAtIdx(i)->getType()));
    }*/
}
    ...
    if (aMenubar->testClassName("Menubar"))
    {
        ...
        while (i<ITEM_COUNT)
        {
            ...
            {   
                // New root item
                // Just call our helper method
                currentRoot = AddNewWindow (aMenubar, look + CEGUI::String ("/MenuItem"), controlName + StringHelper::ToCEGUIString(itemIndex));
                newItem = currentRoot;
            ...
            {   // Add to current root item
                if(currentRoot)
                    newItem = AddNewWindow (currentRoot, look + CEGUI::String ("/MenuItem"), currentRoot->getName() + StringHelper::ToCEGUIString(itemIndex));
                else
                {
...
{
    return (m_background && m_background->getType().find("StaticImage"));
}
...
bool EditorDocument::InitBackground()
{
    ...
    if (!m_background)
    {
        ...
        const CEGUI::String staticImage = FindFirstMapping("StaticImage");
        if (!staticImage.empty())
        {
            m_background = CEGUI::WindowManager::getSingletonPtr()->createWindow("TaharezLook/StaticImage", "BackgroundWindow");
            // Disable frame and standard background
            m_background->setProperty("FrameEnabled", "false");
            m_background->setProperty("BackgroundEnabled", "false");
            // Apply last settings (if this fails, we have at least the required hierarchy).
            SetBackgroundImage(Options::GetInstancePtr()->GetCurrentBackground());
        }
        else
        {   // Stub
            m_background = CEGUI::WindowManager::getSingletonPtr()->createWindow("DefaultWindow", "BackgroundWindow");
...
{   // Only process when a file is given
    if (!fullpath.IsEmpty())
    {   // Remove current imageset. This call just silently bail out when not found.
        ...
        {   // Just to be sure.
            ...
            {   // Okay, behold... because ImageSets get loaded from the predefined resource group,
                ...
                // Don't use INI setting for the visibility. We assume that the user wants to have it visible
                // when he/she sets a new one.
...
{
    // Validations
    wxASSERT_MSG(aLayout != NULL, wxT("Supplied layout is NULL"));
    ...
    // If something goes wrong, show user
    SHOW_EXCEPTION("EditorDocument::SetActiveLayout", "Error", "Unexpected standard exception while setting background image", "Unexpected non-standard exception while setting background image");

    // Note that we store a pointer to the given layout, not the "real" root (the background). This
    // is because we don't want to save the background window together with the layout, since the background
    // exists only runtime.
    m_activeLayout = aLayout;
    (void)CEGUI::System::getSingletonPtr()->setGUISheet (m_activeLayout);	
    // Don't inherit the alpha setting from the background, otherwise it becomes (in)visible with the background setting.
    m_activeLayout->setProperty("InheritsAlpha", "false");
}
...
//------------------------------------------------------------------------
void EditorDocument::HandleMousePressed (const int WXUNUSED(aMouseX), const int WXUNUSED(aMouseY), const bool shiftPressed) 
{
    ...
    if (m_hoveredWindow) 
    {
        ...
        // Call helper method
        SelectWindow (m_hoveredWindow, !shiftPressed); 
    }
    else
    {
        HandleColoursUpdate(shiftPressed);
    }
...
//------------------------------------------------------------------------
void EditorDocument::SelectWindow (CEGUI::Window* aWindow, const bool resetSelection)
{
    ...
    // Validations
    wxASSERT_MSG(aWindow != NULL, NULL_WINDOW_MSG);

    //LogDebug(wxT("Reset? %d"), (int) resetSelection);
    // Not necessary when already selected and not toggled
    if (m_selection.IsSelected(aWindow) && resetSelection)
    {
        return;
    }
    ...
    {	// We unselected the window. Use the first selection for the notification.
        ...
        {	// Ptr to window inside the box
            newSelectedWindow = m_selection.GetBoxes().begin()->GetWindow();
        }
        // else stays NULL
    ...
    if(newSelectedWindow)
    {
        ...
        // an invisible item, just show it.
        // if it's in a pane show the pane.
        if (!newSelectedWindow->isVisible ())
        {
            ...
            {          
                ...
                if (currWindow->getParent() && 
                    currWindow->getParent()->getName().find("__auto_TabPane__"))
                {
                    // hide brother tab
...
//------------------------------------------------------------------------
COMPILE_FIXME("This function is faulty and must take in account the window children. As is, the 'Select All' command cannot be used.")
...
//------------------------------------------------------------------------
bool EditorDocument::OnSaveDocument (const wxString& layoutname)
{
    // Validations
    ...
    std::ofstream file ;
    file.open (wxConvLibc.cWX2MB(layoutname), std::ios::out) ;
    if (!file.is_open ()) 
    {
...
void EditorDocument::CopySelection()
{
    ...
    // One by one, add deep copies.
    for (Selection::Boxes::iterator boxIt = m_selection.GetBoxes().begin(); boxIt != m_selection.GetBoxes().end(); ++boxIt)
    {
        m_copiedSelection.AddRecursiveCopy(boxIt->GetWindow());
    }
}
...
void EditorDocument::PasteCopiedSelection()
{
    ...
    if (m_selection.Size()) 
    {
        m_copiedSelection.PasteSelection(m_selection.GetBoxes().begin()->GetWindow());
    }
    else 
    {
        m_copiedSelection.PasteSelection(m_activeLayout);
    }
    COMPILE_TODO("The pasted elements become the new selection")
    //m_selection.clearSelection();
    // Tell observers
    for (size_t i = 0; i < m_observers.size(); ++i) 
    {
...
void EditorDocument::DeleteSelection()
{
    ...
    while (m_selection.Size() > 0)
    {
        // Destroy it. Only notify on the last one.
        if (!RemoveWindow(m_selection.GetBoxes().begin()->GetWindow(), m_selection.Size() == 1))
        {
            break;
        }
    }

    // Little easy: select the layout.
    if (m_activeLayout)
    {
