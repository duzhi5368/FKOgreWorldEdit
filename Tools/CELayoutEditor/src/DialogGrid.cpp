...
wxDialog(parent, wxID_ANY, wxT("网格设置"), wxDefaultPosition, wxSize(300, 255))
{
...
void DialogGrid::Setup()
{
    ...
    wxStaticBox* const mainGroup = new wxStaticBox(this, wxID_ANY, wxT("更新设置:"));
    m_visible = new wxCheckBox(this, wxID_ANY, wxT("显示网格") );

    // Snap options
    const wxString options[] = {wxT("吸附 X"), wxT("吸附 Y"), wxT("两者都吸附")};
    m_snapModeChoice = new wxRadioBox(this, wxID_ANY, wxT("吸附模式:"), wxDefaultPosition, wxDefaultSize, 3, options);

    wxStaticText* const size = new wxStaticText(this, wxID_ANY, wxT("大小(像素值):") );
    // Validate for positive number
    m_size = new wxTextCtrl(this, wxID_ANY);

    wxButton* const ok = new wxButton(this, ID_OK_BUTTON, wxT("确定(&O)") );
    wxButton* const cancel = new wxButton(this, ID_CANCEL_BUTTON, wxT("取消(&C)") );
