...
{
    // Validations
    wxCHECK_RET(m_parentWindow != NULL, wxT("Invalid or empty parent window"));

    // All "groups". Put the parent's name + type in the text field.  
    wxStaticBox* const parentGroup = new wxStaticBox(this, wxID_ANY, StringHelper::ToWXString(m_parentWindow->getName() + "(" + m_parentWindow->getType() +  "):"));

    // All static fields
    wxStaticText* const staticName = new wxStaticText(this, wxID_ANY, wxT("新窗口的名字:"));
    wxStaticBox* const staticWidgets = new wxStaticBox(this, wxID_ANY, wxT("可用的窗口类型:"));

    // All edit fields. Default the name to "parentName/"
    m_editName = new wxTextCtrl(this, wxID_ANY, StringHelper::ToWXString(m_parentWindow->getName() + "/"));
    ...
    const wxString options[] = {wxT("显示所有"), wxT("显示常用的")};
    m_filterBox = new wxRadioBox(this, ID_FILTER_BOX, wxT("窗口类型筛选:"), wxDefaultPosition, wxDefaultSize, 2, options);
    ...
    // it will be handled internally by wxWidgets wxSizer::Add()
    wxStaticText* const filterWarning = new wxStaticText(this, wxID_ANY, wxT("蓝色的是需要父窗口类型的"));
    ...
    m_okButton = new wxButton(this, wxID_OK, wxT("确定(&O)"));
    // Disable until a widget gets selected.
    (void)m_okButton->Enable(false);
    (void)buttonSizer->Add(m_okButton, 0, wxEXPAND | wxALL, BORDER);
    (void)buttonSizer->Add(new wxButton(this, wxID_CANCEL, wxT("取消(&C)")), 0, wxEXPAND | wxALL, BORDER);
...
void DialogAddWindow::InitWidgetTree(const CEGUI::Window* const aParentWindow)
{
    ...
    const wxTreeItemId rootNode = m_widgetTree->AddRoot(wxT("窗口类型"));
...
bool DialogAddWindow::Filter(const CEGUI::String& aWidget, const CEGUI::Window* const aParentWindow) const
{
    ...
    //LogDebug ( wxT("Filter - widget: '%s', parent: '%s'"), StringHelper::ToWXString(aWidget), StringHelper::ToWXString(aParentWindow->getType()));
