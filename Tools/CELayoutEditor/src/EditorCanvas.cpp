...
void EditorCanvas::OnLeftDown(wxMouseEvent& event)
{
    ...
    {
        EditorDocument* const document = wx_static_cast(EditorDocument*, m_view->GetDocument());
        document->HandleMousePressed (event.GetX(), event.GetY(), event.ShiftDown() || event.ControlDown()) ;
    }
}
...
void EditorCanvas::DrawResizers() const
{
    ...
    for (Selection::Boxes::iterator boxIt = document->GetSelection()->GetBoxes().begin(); boxIt != document->GetSelection()->GetBoxes().end(); ++boxIt)
    {
...
void EditorCanvas::RenderLayout() const
{
    if (CEGUI::System::getSingletonPtr() != NULL)
    {
        ...
        if (CEGUI::System::getSingleton().getRenderer())
        {
            ...
            if (CEGUI::WindowManager::getSingleton().isWindowPresent("BackgroundWindow"))
            {
                CEGUI::WindowManager::getSingleton().getWindow("BackgroundWindow")->render();
            }
