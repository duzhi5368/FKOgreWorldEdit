...
void ElasticBox::moveLeftEdge( float delta )
{
    ...
    // calculate adjustment as pixel aligned value
    float adjustment = delta / getParentPixelWidth();

    URect area( d_area );
    area.d_min.d_x.d_scale += adjustment;
    setArea( area );
}
...
void ElasticBox::moveRightEdge( float delta )
{
    ...
    URect area( d_area );
    area.d_max.d_x.d_scale += adjustment;
    setArea( area );
...
void ElasticBox::moveTopEdge( float delta )
{
    ...
    // calculate adjustment as pixel aligned value
    float adjustment = delta / getParentPixelHeight();

    URect area( d_area );
    area.d_min.d_y.d_scale += adjustment;
    setArea( area );
}
...
void ElasticBox::moveBottomEdge( float delta )
{
    ...
    URect area( d_area );
    area.d_max.d_y.d_scale += adjustment;
    setArea( area );
...
{
    // find out which edge the mouse is on
    FrameWindow::SizingLocation dragEdge = getSizingBorderAtPoint( d_dragPoint );
...
void ElasticBox::doMovingUpdate( const CEGUI::Point& mousePosition )
{
    ...
    // calculate adjustments as relative values
    float adjustment_x = delta_x  / getParentPixelWidth();
    float adjustment_y = delta_y  / getParentPixelHeight();

    // calculate offset in relative co-ords (so that this will scale properly)
    UVector2 offset(
        cegui_reldim( adjustment_x ),
        cegui_reldim( adjustment_y ) );
...
void ElasticBox::onMouseButtonDown( CEGUI::MouseEventArgs& e )
{
    ...
    if ( e.button == LeftButton )
    {
        ...
        FrameWindow::SizingLocation szLoc = getSizingBorderAtPoint( localPos );

        if ( isSizable() && ( szLoc != FrameWindow::SizingNone ) )
        {
        ...
        else if ( isMovable() && ( szLoc == FrameWindow::SizingNone ) )
        {
            // ensure all inputs come to us for now
            if ( captureInput() )
            {
