    ...
    m_backgroundColour( 255, 170, 200 )
{
    ...
    m_resGrpsEditor = new DialogResourceGroups(
        this, 0, wxT( "资源组设置" ), wxDefaultPosition, wxSize( 512, 480 ) );
}
...
void EditorFrame::AttachMenubar()
{
    ...
    menu_file->Append( wxID_NEW, wxT( "新建\tCtrl-N" ), wxT( "Create a new document." ) );
    menu_file->Append( wxID_OPEN, wxT( "载入...\tCtrl-L" ), wxT( "Open an existing document." ) );
    menu_file->Append( wxID_SAVEAS, wxT( "保存...\tCtrl-S" ), wxT( "Save the active document." ) );
    menu_file->Append( wxID_CLOSE, wxT( "关闭 \tCtrl-K" ), wxT( "Close the active document." ) );
    menu_file->AppendSeparator();
    menu_file->Append( wxID_EXIT, wxT( "退出\tCtrl-Q" ), wxT( "Quit the application; prompts to save document." ) );
    menu_bar->Append( menu_file, wxT( "文件" ) );

    wxMenu* menu_options = new wxMenu;
    menu_options->Append( ID_MENU_BACKGROUND_COLOR, wxT( "设置背景颜色..." ), wxT( "choice the colour of the background." ) );
    menu_options->AppendSeparator();
    menu_options->Append( ID_MENU_RESOURCE_GROUPS, wxT( "资源组设置..." ), wxT( "View and edit currently defined resource groups and directories." ) );
    menu_bar->Append( menu_options, wxT( "选项" ) );

    wxMenu* menu_help = new wxMenu;
    menu_help->Append( wxID_HELP, wxT( "帮助..." ), wxT( "" ) );
    menu_help->Append( wxID_ABOUT, wxT( "关于..." ), wxT( "Display program information, version number and copyright." ) );
