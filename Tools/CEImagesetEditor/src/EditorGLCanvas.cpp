...
void EditorGLCanvas::initialiseCEGUI()
{
    ...
    String dataDir( STRINGIZE( EDITOR_DATA_DIR ) );
...
void EditorGLCanvas::setRegionArea( const wxString& name, const wxRect& dim )
{
    if ( m_imagesetStaticImage->isChild( CEGUIHelper::ToCEGUIString( name ) ) )
    {
        ...
        float posx = static_cast<float>( dim.GetX() ) / m_realWidth;
        float posy = static_cast<float>( dim.GetY() ) / m_realHeight;
        float width = static_cast<float>( dim.GetWidth() ) / m_realWidth;
        float height = static_cast<float>( dim.GetHeight() ) / m_realHeight;
        win->setPosition( UVector2( UDim( posx, 0.0f ), UDim( posy, 0.0f ) ) );
        win->setSize( UVector2( UDim( width, 0.0f ), UDim( height, 0.0f ) ) );
...
bool EditorGLCanvas::handleRegionModified( const EventArgs& e )
{
    ...
    if ( isDocumentValid() )
    {
        ...
        {
            wxString name = CEGUIHelper::ToWXString( wnd->getName() );
            float posx    = wnd->getXPosition().asRelative( wnd->getParentPixelWidth() ) * m_realWidth;
            float posy    = wnd->getYPosition().asRelative( wnd->getParentPixelHeight() ) * m_realHeight;
            float width   = wnd->getWidth().asRelative( wnd->getParentPixelWidth() ) * m_realWidth;
            float height  = wnd->getHeight().asRelative( wnd->getParentPixelHeight() ) * m_realHeight;
