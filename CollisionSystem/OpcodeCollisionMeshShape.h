...
namespace Ogre
{
	...
	*/
	class OpcodeCollisionMeshShape : public OpcodeCollisionShape
	{
	public:

		/** 
		*/
		OpcodeCollisionMeshShape();

		/** 
		*/
		virtual ~OpcodeCollisionMeshShape();

		/** Initialize this shape using the mesh and scale provided
		*/
		bool init(const String& meshName, const Vector3& position = Vector3::ZERO, const Quaternion& orient = Quaternion::IDENTITY, const Vector3& scale = Vector3::UNIT_SCALE);
	...
	protected:
		...
		Returns success
		*/
		bool initFromMesh( const Mesh* const mesh, size_t& vertexCount, IceMaths::Point*& vertices, size_t& indexCount, unsigned long*& indices, const Vector3& position = Vector3::ZERO, const Quaternion& orient = Quaternion::IDENTITY, const Vector3& scale = Vector3::UNIT_SCALE);
